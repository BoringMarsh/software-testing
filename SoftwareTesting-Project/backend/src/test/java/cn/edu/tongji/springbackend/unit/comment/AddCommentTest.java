...
public class AddCommentTest {
    ...
    @Issue("AUTH-123")
    @TmsLink("TMS-456")
    @Sql(scripts = "/sql/comment.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
    public void addCommentTest(AddCommentTestCase testCase) {
        ...
        try {
            ...
            System.out.println(e.getMessage().subSequence(0, 20));
            actualOutput = e.getMessage().subSequence(0, 20).toString();
        } catch (DataIntegrityViolationException e) {
            System.out.println(e.getMessage().subSequence(6, 30));
